...
function showPlayOverlay() {
	...
	//	console.log('Error just joke  not jok');
	//	if (webRtcPlayerObj)
	...
	let retryCount = 0;
	const maxRetries = 9999; // ������Դ���
	const retryInterval = 200; // ���Լ��ʱ�䣨���룩

	function tryPlayVideo() {
		if (webRtcPlayerObj && webRtcPlayerObj.video) {
			webRtcPlayerObj.video.muted = true; // ����Ƶ����
			webRtcPlayerObj.video.play().then(() => {
				console.log('Videoplaysuccess');
				webRtcPlayerObj.video.play();
				requestQualityControl();
				showFreezeFrameOverlay();
				// ������ųɹ���ִ���κ���Ҫ�ĳɹ��߼����������ز��Ÿ��ǲ�
				...
				console.error('Videoplayfail:', error);
				// �������ʧ����δ�ﵽ������Դ�����������ʱ�ٴγ���
				if (retryCount < maxRetries) {
					console.log(`Videoplayrepeat����${retryCount + 1}�Σ�`);
					setTimeout(tryPlayVideo, retryInterval);
					retryCount++;
				} else {
					console.log('Videoplayfinish');
					// �ﵽ������Դ�����ִ���κ���Ҫ��ʧ���߼�
				}
			});
		}
...
function setupWebRtcPlayer(htmlElement, config) {
	...
	webRtcPlayerObj.onDataChannelMessage = function (data) {
		...
		if (freezeFrame.receiving) {
			...
			console.warn(`unrecognized data received, packet ID ${view[0]}`);
		}
	};
